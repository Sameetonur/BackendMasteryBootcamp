...
public class ProductManager : IProductService
{
    ...
    public async Task<ResponseDto<ProductDto>> GetAsync(int id)
    {
        ...
        {
            var products = await _productRepository.GetAllAsync(
                predicate: x => x.Id == id && x.IsActive == true, // isimlendirme tekniği ile karışık yazabbiliriz ama null olanı da belirtmek gerekir.
                orderBy: x => x.OrderByDescending(x => x.CreateDate),
                includes: query => query.Include(x => x.ProductCategories).ThenInclude(x => x.Category)
                );
            ...
            if (!products.Any())
            {
                return ResponseDto<ProductDto>.Fail("Herhangi bir ürün bulunamadı", StatusCodes.Status404NotFound);

            }
            var productDtos = _mapper.Map<ProductDto>(products);
            return ResponseDto<ProductDto>.Success(productDtos, StatusCodes.Status200OK);
        }
        catch (System.Exception ex)
        {
