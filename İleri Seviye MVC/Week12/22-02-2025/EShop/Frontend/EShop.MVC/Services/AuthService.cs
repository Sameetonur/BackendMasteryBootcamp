...
namespace EShop.MVC.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<ResponseModel<TokenModel>> LoginAsync(LoginModel loginModel)
        {
            ...
            //eğer başarılı bir şekilde login olunmuşsa token'ı cookie'ye ekleyelim.
            if (response!.IsSuccessful == true && response.Data != null)
            {
                ...
                var authProperties = new AuthenticationProperties
                {
                    ...
                    // IsPersistent = loginModel.RememberMe //beni hatırla kısmı da buraya eklenebilir.
                    //beni hatırla kısmı da buraya eklenebilir.
                };
        ...
        {
            var response = await _httpClientService.PostAsync<ResetPasswordModel, ResponseModel<NoContent>>("auths/register", resetPasswordModel);
            return response!;
        }
        ...
        public async Task LogoutAsync()
        {
            ...
            //bu logout işlemi  client tarafında çıkış yapmak için kullanılır. Api tarafından dağıtılan  ve buraya gönderilmiş olan(her ne kadar artık bu client uygulama içinde yok olmuş olsa da) token, aslında hala geçerliliğini koruyor ta ki sona erme tarihine kadar.
        }
    }
}
